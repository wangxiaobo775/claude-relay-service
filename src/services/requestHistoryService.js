...
class RequestHistoryService {
  constructor() {
    ...
    this.maxResponseBodySize = 50000; // 最大存储的响应体大小
  }
  ...
  async completeRequest(requestId, responseData) {
    ...
    try {
      ...
      const updatedRecord = {
        ...
        totalTokens: (responseData.inputTokens || 0) + (responseData.outputTokens || 0) + 
                    (responseData.cacheCreateTokens || 0) + (responseData.cacheReadTokens || 0),
        endTime
      };
  ...
  async failRequest(requestId, errorData) {
    ...
    try {
      ...
      logger.debug(`❌ Request failed: ${requestId}, duration: ${duration}ms, error: ${errorData.error}`);
    } catch (error) {
  ...
  setMaxSizes(requestBodySize, responseBodySize) {
    ...
    logger.info(`📏 Max sizes updated: request=${this.maxRequestBodySize}, response=${this.maxResponseBodySize}`);
  }
}
